...
void hfs_setsilence(int s) { silence = s; }

void writeToFile(HFSPlusCatalogFile *file, AbstractFile *output,
                 Volume *volume) {
...
void grow_hfs(Volume *volume, uint64_t newSize) {
  ...
  if (volume->volumeHeader->allocationFile.logicalSize < newMapSize) {
    if (volume->volumeHeader->freeBlocks <
        ((newMapSize - volume->volumeHeader->allocationFile.logicalSize) /
         volume->volumeHeader->blockSize)) {
  ...
  /* don't need to increment freeBlocks because we will allocate another
   * alternate volume header later on */
...
void removeAllInFolder(HFSCatalogNodeID folderID, Volume *volume,
  ...
  while (list != NULL) {
    ...
    if (isRoot && (name[0] == '\0' ||
                   strncmp(name, ".HFS+ Private Directory Data",
                           sizeof(".HFS+ Private Directory Data") - 1) == 0)) {
      free(name);
      list = list->next;
      continue;
    }
...
void addAllInFolder(HFSCatalogNodeID folderID, Volume *volume,
                    const char *parentName) {
  CatalogRecordList *list;
  CatalogRecordList *theList;
  char cwd[1024];
  ...
  while ((ent = readdir(dir)) != NULL) {
    ...
    while (list != NULL) {
      ...
      if (strcmp(name, ent->d_name) == 0) {
        cnid = (list->record->recordType == kHFSPlusFolderRecord)
                   ? (((HFSPlusCatalogFolder *)list->record)->folderID)
    ...
    if ((tmp = opendir(ent->d_name)) != NULL) {
      ...
      if (strncmp(fullName, "/Applications/", sizeof("/Applications/") - 1) ==
          0) {
        testBuffer[0] = '\0';
        ...
        if (strcmp(testBuffer, fullName) == 0) {
          if (strcmp(ent->d_name, "Installer") == 0 ||
            ...
            chmodFile(fullName, 0755, volume);
                 ...
                 strncmp(fullName, "/Applications/BootNeuter.app/bin/",
                         sizeof("/Applications/BootNeuter.app/bin/") - 1) ==
                     0 ||
                 strncmp(fullName, "/sbin/", sizeof("/sbin/") - 1) == 0 ||
                 strncmp(fullName, "/usr/sbin/", sizeof("/usr/sbin/") - 1) ==
                     0 ||
                 strncmp(fullName, "/usr/bin/", sizeof("/usr/bin/") - 1) == 0 ||
                 strncmp(fullName, "/usr/libexec/",
                         sizeof("/usr/libexec/") - 1) == 0 ||
                 strncmp(fullName, "/usr/local/bin/",
                         sizeof("/usr/local/bin/") - 1) == 0 ||
                 strncmp(fullName, "/usr/local/sbin/",
                         sizeof("/usr/local/sbin/") - 1) == 0 ||
                 strncmp(fullName, "/usr/local/libexec/",
                         sizeof("/usr/local/libexec/") - 1) == 0) {
        chmodFile(fullName, 0755, volume);
        printf("Giving permissions to %s\n", fullName);
        fflush(stdout);
      }
...
void extractAllInFolder(HFSCatalogNodeID folderID, Volume *volume) {
  ...
  while (list != NULL) {
    name = unicodeToAscii(&list->name);
    if (strncmp(name, ".HFS+ Private Directory Data",
                sizeof(".HFS+ Private Directory Data") - 1) == 0 ||
        name[0] == '\0') {
      free(name);
      list = list->next;
...
void addall_hfs(Volume *volume, const char *dirToMerge, const char *dest) {
  ...
  if (record != NULL) {
    if (record->recordType == kHFSPlusFolderRecord)
      addAllInFolder(((HFSPlusCatalogFolder *)record)->folderID, volume,
                     initPath);
    else {
      printf("Not a folder\n");
      exit(0);
...
int copyAcrossVolumes(Volume *volume1, Volume *volume2, char *path1,
                      char *path2) {
  void *buffer;
  size_t bufferSize;
  ...
  if (!silence) {
    printf("writing (%ld)... ", (long)tmpFile->getLength(tmpFile));
    fflush(stdout);
  }
...
void displayFolder(HFSCatalogNodeID folderID, Volume *volume) {
  ...
  while (list != NULL) {
    if (list->record->recordType == kHFSPlusFolderRecord) {
      ...
      if (file->permissions.ownerFlags & UF_COMPRESSED) {
        attrSize = getAttribute(volume, file->fileID, "com.apple.decmpfs",
    ...
    if (date != NULL) {
      printf("%2d/%2d/%4d %02d:%02d ", date->tm_mon, date->tm_mday,
...
void displayFileLSLine(Volume *volume, HFSPlusCatalogFile *file,
                       const char *name) {
  time_t fileTime;
  struct tm *date;
  HFSPlusDecmpfs *compressData;
  ...
  if (file->permissions.ownerFlags & UF_COMPRESSED) {
    getAttribute(volume, file->fileID, "com.apple.decmpfs",
                 (uint8_t **)(&compressData));
    flipHFSPlusDecmpfs(compressData);
    printf("%12" PRId64 " ", compressData->size);
    free(compressData);
  ...
  date = localtime(&fileTime);
  if (date != NULL) {
    printf("%2d/%2d/%4d %2d:%02d ", date->tm_mon, date->tm_mday,
           date->tm_year + 1900, date->tm_hour, date->tm_min);
  } else {
    printf("                 ");
  }
...
void hfs_ls(Volume *volume, const char *path) {
  ...
  printf(
      "Total filesystem size: %d, free: %d\n",
      (volume->volumeHeader->totalBlocks - volume->volumeHeader->freeBlocks) *
          volume->volumeHeader->blockSize,
      volume->volumeHeader->freeBlocks * volume->volumeHeader->blockSize);
...
void hfs_untar(Volume *volume, AbstractFile *tarFile) {
  ...
  while (curRecord < tarSize) {
    ...
    HFSPlusCatalogRecord *record = getRecordFromPath3(
        fileName, volume, NULL, NULL, TRUE, FALSE, kHFSRootFolderID);
    if (record) {
