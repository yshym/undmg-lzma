...
static int writeBTNodeDescriptor(BTNodeDescriptor *descriptor, uint32_t num,
                                 BTree *tree) {
  ...
  if (!WRITE(tree->io, num * tree->headerRec->nodeSize,
             sizeof(BTNodeDescriptor), &myDescriptor))
    return FALSE;
...
static int writeBTHeaderRec(BTree *tree) {
  ...
  if (!WRITE(tree->io, sizeof(BTNodeDescriptor), sizeof(BTHeaderRec),
             &headerRec)) {
    return FALSE;
  }
...
off_t getRecordOffset(int num, uint32_t nodeNum, BTree *tree) {
  ...
  if (!READ(tree->io, nodeOffset + tree->headerRec->nodeSize -
                          (sizeof(uint16_t) * (num + 1)),
...
static off_t getFreeSpace(uint32_t nodeNum, BTNodeDescriptor *descriptor,
                          BTree *tree) {
  uint16_t num;
  off_t nodeOffset;
  ...
  nodeOffset = nodeNum * tree->headerRec->nodeSize;
  freespaceOffsetOffset =
      nodeOffset + tree->headerRec->nodeSize - (sizeof(uint16_t) * (num + 1));
...
static void *searchNode(BTree *tree, uint32_t root, BTKey *searchKey,
                        int *exact, uint32_t *nodeNumber, int *recordNumber) {
  BTNodeDescriptor *descriptor;
  BTKey *key;
  off_t recordOffset;
  ...
  for (i = 0; i < descriptor->numRecords; i++) {
    ...
    if (res == 0) {
      if (descriptor->kind == kBTLeafNode) {
        ...
        free(descriptor);
        return searchNode(
            tree, getNodeNumberFromPointerRecord(recordDataOffset, tree->io),
            searchKey, exact, nodeNumber, recordNumber);
      }
    } else if (res > 0) {
      break;
    }
  ...
  if (descriptor->kind == kBTLeafNode) {
    ...
    free(descriptor);
    return searchNode(
        tree, getNodeNumberFromPointerRecord(lastRecordDataOffset, tree->io),
        searchKey, exact, nodeNumber, recordNumber);
  } else {
    if (nodeNumber != NULL) {
      *nodeNumber = root;
...
void *search(BTree *tree, BTKey *searchKey, int *exact, uint32_t *nodeNumber,
             int *recordNumber) {
  return searchNode(tree, tree->headerRec->rootNode, searchKey, exact,
                    nodeNumber, recordNumber);
}

static uint32_t linearCheck(uint32_t *heightTable, unsigned char *map,
                            BTree *tree, uint32_t *errCount) {
  uint8_t i;
  uint8_t j;
  uint32_t node;
    ...
    if (node != 0) {
      ...
      if (i == 1) {
        if (node != tree->headerRec->firstLeafNode) {
          printf("BTREE CONSISTENCY ERROR: First leaf node (%d) is not "
                 "correct. Should be: %d\n",
      ...
      while (node != 0) {
        ...
        if (descriptor->bLink != prevNode) {
          printf("BTREE CONSISTENCY ERROR: Node %d is not properly linked with "
                 "previous node %d\n",
                 node, prevNode);
          (*errCount)++;
        }
        ...
        if (descriptor->height != i) {
          printf("BTREE CONSISTENCY ERROR: Node %d (%d) is not properly linked "
                 "with nodes of the same height %d\n",
        ...
        /*if(descriptor->kind == kBTIndexNode && descriptor->numRecords < 2) {
          printf("BTREE CONSISTENCY ERROR: Node %d does not have at least two
        children\n", node);
          (*errCount)++;
        }*/
        ...
        for (j = 0; j < descriptor->numRecords; j++) {
          ...
          if (previousKey != NULL) {
            if (COMPARE(tree, key, previousKey) < 0) {
              printf("BTREE CONSISTENCY ERROR: Ordering not preserved during "
                     "linear check for record %d node %d: ",
                     j, node);
              (*errCount)++;
      ...
      if (i == 1) {
        if (prevNode != tree->headerRec->lastLeafNode) {
          printf("BTREE CONSISTENCY ERROR: Last leaf node (%d) is not correct. "
                 "Should be: %d\n",
  ...
  if (leafRecords != tree->headerRec->leafRecords) {
    printf("BTREE CONSISTENCY ERROR: leafRecords (%d) is not correct. Should "
           "be: %d\n",
...
static uint32_t traverseNode(uint32_t nodeNum, BTree *tree, unsigned char *map,
                             int parentHeight, BTKey **firstKey,
                             BTKey **lastKey, uint32_t *heightTable,
                             uint32_t *errCount, int displayTree) {
  BTNodeDescriptor *descriptor;
  BTKey *key;
  BTKey *previousKey;
  BTKey *retFirstKey;
  ...
  if (nodeNum == tree->headerRec->rootNode) {
    if (descriptor->height != tree->headerRec->treeDepth) {
      printf("BTREE CONSISTENCY ERROR: Root node %d (%d) does not have the "
             "proper height (%d)\n",
             nodeNum, descriptor->height, tree->headerRec->treeDepth);
      fflush(stdout);
      (*errCount)++;
    }
  } else {
    if (descriptor->height != (parentHeight - 1)) {
      printf(
          "BTREE CONSISTENCY ERROR: Node %d does not have the proper height\n",
          nodeNum);
      fflush(stdout);
      (*errCount)++;
    }
  ...
  for (i = 0; i < descriptor->numRecords; i++) {
    ...
    if ((recordDataOffset - (nodeNum * tree->headerRec->nodeSize)) >
        (tree->headerRec->nodeSize -
         (sizeof(uint16_t) * (descriptor->numRecords + 1)))) {
      printf("BTREE CONSISTENCY ERROR: Record data extends past offsets in "
             "node %d record %d\n",
             nodeNum, i);
      fflush(stdout);
      (*errCount)++;
    ...
    if (previousKey != NULL) {
      ...
      if (res < 0) {
        printf("BTREE CONSISTENCY ERROR(traverse): Ordering between records "
               "within node not preserved in record %d node %d for ",
               i, nodeNum);
        (*errCount)++;
        tree->keyPrint(previousKey);
        printf(" < ");
    ...
    if (descriptor->kind == kBTIndexNode) {
      count += traverseNode(
          getNodeNumberFromPointerRecord(recordDataOffset, tree->io), tree, map,
          descriptor->height, &retFirstKey, &retLastKey, heightTable, errCount,
          displayTree);

      if (COMPARE(tree, retFirstKey, key) != 0) {
        printf("BTREE CONSISTENCY ERROR: Index node key does not match first "
               "key in record %d node %d\n",
               i, nodeNum);
        fflush(stdout);
        (*errCount)++;
      }
      if (COMPARE(tree, retLastKey, key) < 0) {
        printf("BTREE CONSISTENCY ERROR: Last key is less than the index node "
               "key in record %d node %d\n",
               i, nodeNum);
        fflush(stdout);
        (*errCount)++;
    ...
    if (recordOffset < lastrecordDataOffset) {
      printf("BTREE CONSISTENCY ERROR: Record offsets are not in order in node "
             "%d starting at record %d\n",
             nodeNum, i);
      fflush(stdout);
      (*errCount)++;
    }
...
static unsigned char *mapNodes(BTree *tree, uint32_t *numMapNodes,
                               uint32_t *errCount) {
  unsigned char *map;
  ...
  *numMapNodes = 0;
  ...
  while (TRUE) {
    ...
    if (mapNode == 0) {
      printf("BTREE CONSISTENCY ERROR: Not enough map nodes allocated! "
             "Allocated for: %d, needed: %d\n",
  ...
  if (freeNodes != tree->headerRec->freeNodes) {
    printf("BTREE CONSISTENCY ERROR: Free nodes %d differ from actually "
           "allocated %d\n",
...
int debugBTree(BTree *tree, int displayTree) {
  ...
  fflush(stdout);
  heightTable =
      (uint32_t *)malloc(sizeof(uint32_t) * (tree->headerRec->treeDepth + 1));
  for (i = 0; i <= tree->headerRec->treeDepth; i++) {
    heightTable[i] = 0;
  }
  ...
  if (tree->headerRec->rootNode == 0) {
    if (tree->headerRec->firstLeafNode == 0 &&
      ...
      printf("BTREE CONSISTENCY ERROR: First leaf node (%d) and last leaf node "
             "(%d) inconsistent with empty BTree\n",
    ...
    fflush(stdout);
    traverseCount =
        traverseNode(tree->headerRec->rootNode, tree, map, 0, &retFirstKey,
                     &retLastKey, heightTable, &errorCount, displayTree);
  ...
  if (traverseCount != linearCount) {
    printf("BTREE CONSISTENCY ERROR: Linear count and traverse count are "
           "inconsistent\n");
  }

  if (traverseCount != (tree->headerRec->totalNodes -
                        tree->headerRec->freeNodes - numMapNodes - 1)) {
    printf("BTREE CONSISTENCY ERROR: Free nodes and total nodes (%d) and "
           "traverse count are inconsistent\n",
      ...
      if (byte != 0xFF) {
        for (i = 0; i < 8; i++) {
          if ((byte & (1 << (7 - i))) == 0) {
            ...
            ASSERT(WRITE(tree->io, mapRecordStart + byteNumber, 1, &byte),
                   "WRITE");
...
static int markUsed(uint32_t node, BTree *tree) {
  ...
  ASSERT(READ(tree->io, mapNode * tree->headerRec->nodeSize + 14 + byteNumber,
              1, &byte),
         "READ");
  byte |= (1 << (7 - (node % 8)));
  ASSERT(WRITE(tree->io, mapNode * tree->headerRec->nodeSize + 14 + byteNumber,
               1, &byte),
         "WRITE");
...
static int growBTree(BTree *tree) {
  ...
  allocate((RawFile *)(tree->io->data),
           ((RawFile *)(tree->io->data))->forkData->logicalSize +
               ((RawFile *)(tree->io->data))->forkData->clumpSize);
  increasedNodes = (((RawFile *)(tree->io->data))->forkData->logicalSize /
                    tree->headerRec->nodeSize) -
                   tree->headerRec->totalNodes;
  ...
  if (byteNumber < (tree->headerRec->nodeSize - 256)) {
    ...
    while (TRUE) {
      ...
      if (descriptor->fLink == 0) {
        ...
        ASSERT(writeBTNodeDescriptor(descriptor, mapNode, tree),
               "writeBTNodeDescriptor");
        ...
        ASSERT(writeBTNodeDescriptor(&newDescriptor, descriptor->fLink, tree),
               "writeBTNodeDescriptor");

        newNodeOffset = descriptor->fLink * tree->headerRec->nodeSize;

        ASSERT(WRITE(tree->io, newNodeOffset + 14,
                     tree->headerRec->nodeSize - 20, buffer),
               "WRITE");
        offset = 14;
        FLIPENDIAN(offset);
        ASSERT(WRITE(tree->io, newNodeOffset + tree->headerRec->nodeSize - 2,
                     sizeof(offset), &offset),
               "WRITE");
        offset = 14 + tree->headerRec->nodeSize - 20;
        FLIPENDIAN(offset);
        ASSERT(WRITE(tree->io, newNodeOffset + tree->headerRec->nodeSize - 4,
                     sizeof(offset), &offset),
               "WRITE");
...
static uint32_t removeNode(BTree *tree, uint32_t node) {
  ...
  if (descriptor->bLink != 0) {
    ...
    ASSERT(writeBTNodeDescriptor(oDescriptor, descriptor->bLink, tree),
           "writeBTNodeDescriptor");
    free(oDescriptor);
  }
  ...
  if (descriptor->fLink != 0) {
    ...
    ASSERT(writeBTNodeDescriptor(oDescriptor, descriptor->fLink, tree),
           "writeBTNodeDescriptor");
    free(oDescriptor);
  }
...
static uint32_t splitNode(uint32_t node, BTNodeDescriptor *descriptor,
                          BTree *tree) {
  int nodesToMove;
  ...
  offsetsToMove = (node * tree->headerRec->nodeSize) +
                  tree->headerRec->nodeSize -
                  (sizeof(uint16_t) * (descriptor->numRecords + 1));
  offsetsToMoveLength = sizeof(uint16_t) * (nodesToMove + 1);
  offsetsBuffer = (uint16_t *)malloc(offsetsToMoveLength);
  ASSERT(READ(tree->io, offsetsToMove, offsetsToMoveLength, offsetsBuffer),
         "READ");
  ...
  newDescriptor.reserved = 0;
  ASSERT(writeBTNodeDescriptor(&newDescriptor, newNodeNum, tree),
         "writeBTNodeDescriptor");

  if (newDescriptor.fLink != 0) {
    fDescriptor = readBTNodeDescriptor(newDescriptor.fLink, tree);
    fDescriptor->bLink = newNodeNum;
    ASSERT(writeBTNodeDescriptor(fDescriptor, newDescriptor.fLink, tree),
           "writeBTNodeDescriptor");
    free(fDescriptor);
  }
  ...
  ASSERT(writeBTNodeDescriptor(descriptor, node, tree),
         "writeBTNodeDescriptor");

  ASSERT(WRITE(tree->io, newNodeOffset + 14, toMoveLength, buffer), "WRITE");
  ASSERT(WRITE(tree->io, newNodeOffset + tree->headerRec->nodeSize -
                             (sizeof(uint16_t) * (nodesToMove + 1)),
               offsetsToMoveLength, offsetsBuffer),
         "WRITE");

  // The offset for the existing descriptor's new numRecords will happen to be
  // where the old data was, which is now where the free space starts
...
static int moveRecordsDown(BTree *tree, BTNodeDescriptor *descriptor,
                           int record, uint32_t node, int length,
                           int moveOffsets) {
  ...
  ASSERT(READ(tree->io, firstRecordStart, lastRecordEnd - firstRecordStart,
              records),
         "READ");
  firstOffsetStart = (node * tree->headerRec->nodeSize) +
                     tree->headerRec->nodeSize -
                     (sizeof(uint16_t) * (descriptor->numRecords + 1));
  lastOffsetEnd = (node * tree->headerRec->nodeSize) +
                  tree->headerRec->nodeSize - (sizeof(uint16_t) * record);

  offsets = (uint16_t *)malloc(lastOffsetEnd - firstOffsetStart);
  ASSERT(READ(tree->io, firstOffsetStart, lastOffsetEnd - firstOffsetStart,
              offsets),
         "READ");
  ...
  ASSERT(WRITE(tree->io, firstRecordStart + length,
               lastRecordEnd - firstRecordStart, records),
         "WRITE");

  if (moveOffsets > 0) {
    ASSERT(WRITE(tree->io, firstOffsetStart - sizeof(uint16_t),
                 lastOffsetEnd - firstOffsetStart, offsets),
           "WRITE");
    ...
    ASSERT(WRITE(tree->io, firstOffsetStart, lastOffsetEnd - firstOffsetStart,
                 offsets),
           "WRITE");
  }
...
static int doAddRecord(BTree *tree, uint32_t root, BTKey *searchKey,
                       size_t length, unsigned char *content) {
  BTNodeDescriptor *descriptor;
  BTKey *key;
  off_t recordOffset;
  off_t recordDataOffset;
  ...
  if (i != descriptor->numRecords) {
    ...
    moveRecordsDown(
        tree, descriptor, i, root,
        sizeof(searchKey->keyLength) + searchKey->keyLength + length, 1);

    // then insert ourself
    ASSERT(WRITE_KEY(tree, recordOffset, searchKey, tree->io), "WRITE_KEY");
    ASSERT(WRITE(tree->io, recordOffset + sizeof(searchKey->keyLength) +
                               searchKey->keyLength,
                 length, content),
           "WRITE");
    ...
    FLIPENDIAN(offset);
    ASSERT(WRITE(tree->io,
                 (root * tree->headerRec->nodeSize) +
                     tree->headerRec->nodeSize - (sizeof(uint16_t) * (i + 1)),
                 sizeof(uint16_t), &offset),
           "WRITE");
  } else {
    ...
    ASSERT(WRITE_KEY(tree, recordOffset, searchKey, tree->io), "WRITE_KEY");
    ASSERT(WRITE(tree->io,
                 recordOffset + sizeof(uint16_t) + searchKey->keyLength, length,
                 content),
           "WRITE");

    // write the new free offset
    offset = (recordOffset + sizeof(searchKey->keyLength) +
              searchKey->keyLength + length) -
             (root * tree->headerRec->nodeSize);
    FLIPENDIAN(offset);
    ASSERT(WRITE(tree->io,
                 (root * tree->headerRec->nodeSize) +
                     tree->headerRec->nodeSize -
                     (sizeof(uint16_t) * (descriptor->numRecords + 2)),
                 sizeof(uint16_t), &offset),
           "WRITE");
  }
  ...
  ASSERT(writeBTNodeDescriptor(descriptor, root, tree),
         "writeBTNodeDescriptor");
  ASSERT(writeBTHeaderRec(tree), "writeBTHeaderRec");
...
static int addRecord(BTree *tree, uint32_t root, BTKey *searchKey,
                     size_t length, unsigned char *content, int *callAgain) {
  BTNodeDescriptor *descriptor;
  BTKey *key;
  off_t recordOffset;
  off_t recordDataOffset;
  ...
  if (descriptor->kind == kBTLeafNode) {
    if (freeSpace < (sizeof(searchKey->keyLength) + searchKey->keyLength +
    ...
    if (lastRecordDataOffset == 0) {
      ...
      recordDataOffset = recordOffset + key->keyLength + sizeof(key->keyLength);
      nodeBigEndian =
          getNodeNumberFromPointerRecord(recordDataOffset, tree->io);
      ...
      if (searchKey->keyLength != key->keyLength) {
        if (searchKey->keyLength > key->keyLength &&
          ...
          // very unlikely. We need to split this node before we can resize the
          // key of this index. Do that first, and tell them to call again.
          *callAgain = TRUE;
          return splitNode(root, descriptor, tree);
        }

        moveRecordsDown(tree, descriptor, 1, root,
                        searchKey->keyLength - key->keyLength, 0);
      }
      ...
      ASSERT(WRITE_KEY(tree, recordOffset, searchKey, tree->io), "WRITE_KEY");
      ASSERT(WRITE(tree->io,
                   recordOffset + sizeof(uint16_t) + searchKey->keyLength,
                   sizeof(uint32_t), &nodeBigEndian),
             "WRITE");

      FLIPENDIAN(nodeBigEndian);

      newNode =
          addRecord(tree, nodeBigEndian, searchKey, length, content, callAgain);
    } else {
      newNode = addRecord(
          tree, getNodeNumberFromPointerRecord(lastRecordDataOffset, tree->io),
          searchKey, length, content, callAgain);
    }
    ...
    if (newNode == 0) {
      ...
      if (freeSpace < (sizeof(key->keyLength) + key->keyLength +
                       sizeof(newNodeBigEndian) + sizeof(uint16_t))) {
        newNode = splitNode(root, descriptor, tree);

        if (i < descriptor->numRecords) {
          doAddRecord(tree, root, key, sizeof(newNodeBigEndian),
                      (unsigned char *)(&newNodeBigEndian));
        } else {
          doAddRecord(tree, newNode, key, sizeof(newNodeBigEndian),
                      (unsigned char *)(&newNodeBigEndian));
        }
...
static int increaseHeight(BTree *tree, uint32_t newNode) {
  ...
  oldRootKey =
      READ_KEY(tree, (oldRoot * tree->headerRec->nodeSize) + 14, tree->io);
  newNodeKey =
      READ_KEY(tree, (newNode * tree->headerRec->nodeSize) + 14, tree->io);
  ...
  oldRootOffset = 14;
  newNodeOffset = oldRootOffset + sizeof(oldRootKey->keyLength) +
                  oldRootKey->keyLength + sizeof(uint32_t);
  freeOffset = newNodeOffset + sizeof(newNodeKey->keyLength) +
               newNodeKey->keyLength + sizeof(uint32_t);

  tree->headerRec->rootNode = newRoot;
  tree->headerRec->treeDepth = newDescriptor.height;

  ASSERT(WRITE_KEY(tree, newRoot * tree->headerRec->nodeSize + oldRootOffset,
                   oldRootKey, tree->io),
         "WRITE_KEY");
  FLIPENDIAN(oldRoot);
  ASSERT(WRITE(tree->io,
               newRoot * tree->headerRec->nodeSize + oldRootOffset +
                   sizeof(oldRootKey->keyLength) + oldRootKey->keyLength,
               sizeof(uint32_t), &oldRoot),
         "WRITE");

  ASSERT(WRITE_KEY(tree, newRoot * tree->headerRec->nodeSize + newNodeOffset,
                   newNodeKey, tree->io),
         "WRITE_KEY");
  FLIPENDIAN(newNode);
  ASSERT(WRITE(tree->io,
               newRoot * tree->headerRec->nodeSize + newNodeOffset +
                   sizeof(newNodeKey->keyLength) + newNodeKey->keyLength,
               sizeof(uint32_t), &newNode),
         "WRITE");

  FLIPENDIAN(oldRootOffset);
  ASSERT(WRITE(tree->io, (newRoot * tree->headerRec->nodeSize) +
                             tree->headerRec->nodeSize - (sizeof(uint16_t) * 1),
               sizeof(uint16_t), &oldRootOffset),
         "WRITE");

  FLIPENDIAN(newNodeOffset);
  ASSERT(WRITE(tree->io, (newRoot * tree->headerRec->nodeSize) +
                             tree->headerRec->nodeSize - (sizeof(uint16_t) * 2),
               sizeof(uint16_t), &newNodeOffset),
         "WRITE");

  FLIPENDIAN(freeOffset);
  ASSERT(WRITE(tree->io, (newRoot * tree->headerRec->nodeSize) +
                             tree->headerRec->nodeSize - (sizeof(uint16_t) * 3),
               sizeof(uint16_t), &freeOffset),
         "WRITE");

  ASSERT(writeBTNodeDescriptor(&newDescriptor, tree->headerRec->rootNode, tree),
         "writeBTNodeDescriptor");
  ASSERT(writeBTHeaderRec(tree), "writeBTHeaderRec");
...
int addToBTree(BTree *tree, BTKey *searchKey, size_t length,
  ...
  if (tree->headerRec->rootNode != 0) {
    do {
      callAgain = FALSE;
      newNode = addRecord(tree, tree->headerRec->rootNode, searchKey, length,
                          content, &callAgain);
      if (newNode != 0) {
    ...
    offset = 14;
    freeOffset =
        offset + sizeof(searchKey->keyLength) + searchKey->keyLength + length;

    ASSERT(WRITE_KEY(tree,
                     tree->headerRec->rootNode * tree->headerRec->nodeSize +
                         offset,
                     searchKey, tree->io),
           "WRITE_KEY");
    ASSERT(WRITE(tree->io,
                 tree->headerRec->rootNode * tree->headerRec->nodeSize +
                     offset + sizeof(searchKey->keyLength) +
                     searchKey->keyLength,
                 length, content),
           "WRITE");
    ...
    ASSERT(
        writeBTNodeDescriptor(&newDescriptor, tree->headerRec->rootNode, tree),
        "writeBTNodeDescriptor");
    ASSERT(writeBTHeaderRec(tree), "writeBTHeaderRec");
  }
...
static uint32_t removeRecord(BTree *tree, uint32_t root, BTKey *searchKey,
                             int *callAgain, int *gone) {
  BTNodeDescriptor *descriptor;
  int length;
  int i;
  int res;
  ...
  for (i = 0; i < descriptor->numRecords; i++) {
    ...
    if (res == 0) {
      ...
      if (descriptor->kind == kBTLeafNode) {
        ...
        } // don't need to do that if we're the last record, because our old
        // offset pointer will be the new free space pointer
      ...
      if (lastRecordDataOffset == 0 || descriptor->kind == kBTLeafNode) {
        ...
        nodeToTraverse =
            getNodeNumberFromPointerRecord(lastRecordDataOffset, tree->io);
        break;
      }
    }
  ...
  newNode =
      removeRecord(tree, nodeToTraverse, searchKey, callAgain, &childGone);

  if (childGone) {
    if (i != (descriptor->numRecords - 1)) {
      length = getRecordOffset(i + 1, root, tree) - recordOffset;
      moveRecordsDown(tree, descriptor, i + 1, root, -length, -1);
    } // don't need to do that if we're the last record, because our old offset
    // pointer will be the new free space pointer
    ...
    if (checkForChangedKey) {
      // we will remove the first item in the child node, so our index has to
      // change

      key = READ_KEY(tree, getRecordOffset(0, nodeToTraverse, tree), tree->io);

      if (searchKey->keyLength != key->keyLength) {
        if (key->keyLength > searchKey->keyLength &&
            freeSpace < (key->keyLength - searchKey->keyLength)) {
          // very unlikely. We need to split this node before we can resize the
          // key of this index. Do that first, and tell them to call again.
          *callAgain = TRUE;
          return splitNode(root, descriptor, tree);
        ...
        moveRecordsDown(tree, descriptor, i + 1, root,
                        key->keyLength - searchKey->keyLength, 0);
      }
      ...
      ASSERT(WRITE(tree->io, recordOffset + sizeof(uint16_t) + key->keyLength,
                   sizeof(uint32_t), &nodeToTraverse),
             "WRITE");
      FLIPENDIAN(nodeToTraverse);
...
int removeFromBTree(BTree *tree, BTKey *searchKey) {
  ...
  do {
    callAgain = FALSE;
    newNode = removeRecord(tree, tree->headerRec->rootNode, searchKey,
                           &callAgain, &gone);
    if (newNode != 0) {
      increaseHeight(tree, newNode);
    }
