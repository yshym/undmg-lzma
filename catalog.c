...
#define UNICODE_START                                                          \
  (sizeof(uint16_t) + sizeof(HFSCatalogNodeID) + sizeof(uint16_t))
...
static int catalogCompare(BTKey *vLeft, BTKey *vRight) {
  ...
  if (left->parentID < right->parentID) {
    ...
    if (i < right->nodeName.length) {
      ...
      /* do a safety check on key length. Otherwise, bad things may happen later
       * on when we try to add or remove with this key */
...
static int catalogCompareCS(BTKey *vLeft, BTKey *vRight) {
  ...
  if (left->parentID < right->parentID) {
    ...
    return FastUnicodeCompare(left->nodeName.unicode, left->nodeName.length,
                              right->nodeName.unicode, right->nodeName.length);
  }
}
...
static BTKey *catalogKeyRead(off_t offset, io_func *io) {
  ...
  if (!READ(io, offset + UNICODE_START, key->nodeName.length * sizeof(uint16_t),
            ((unsigned char *)key) + UNICODE_START)) {
    return NULL;
  }

  for (i = 0; i < key->nodeName.length; i++) {
    FLIPENDIAN(key->nodeName.unicode[i]);
    if (key->nodeName.unicode[i] ==
        '/') { /* ugly hack that iPhone seems to do */
  ...
  case kHFSPlusFileThreadRecord:
    ...
    if (!READ(io, offset,
              sizeof(uint16_t) + sizeof(uint16_t) + sizeof(uint32_t) +
                  sizeof(uint16_t) + (sizeof(uint16_t) * nameLength),
              record)) {
      return NULL;
    }
...
HFSPlusCatalogRecord *getRecordByCNID(HFSCatalogNodeID CNID, Volume *volume) {
  ...
  thread = (HFSPlusCatalogThread *)search(volume->catalogTree, (BTKey *)(&key),
                                          &exact, NULL, NULL);
  ...
  record = (HFSPlusCatalogRecord *)search(volume->catalogTree, (BTKey *)(&key),
                                          &exact, NULL, NULL);
...
CatalogRecordList *getFolderContents(HFSCatalogNodeID CNID, Volume *volume) {
  ...
  record = (HFSPlusCatalogThread *)search(tree, (BTKey *)(&key), NULL,
  ...
  while (nodeNumber != 0) {
    ...
    while (recordNumber < descriptor->numRecords) {
      ...
      recordDataOffset =
          recordOffset + currentKey->keyLength + sizeof(currentKey->keyLength);

      if (currentKey->parentID == CNID) {
        item = (CatalogRecordList *)malloc(sizeof(CatalogRecordList));
        item->name = currentKey->nodeName;
        item->record =
            (HFSPlusCatalogRecord *)READ_DATA(tree, recordDataOffset, tree->io);

        if (item->record->recordType == kHFSPlusFileRecord &&
            (((HFSPlusCatalogFile *)item->record)->userInfo.fileType) ==
                kHardLinkFileType) {
          sprintf(pathBuffer, "iNode%d", ((HFSPlusCatalogFile *)item->record)
                                             ->permissions.special.iNodeNum);
          nkey.parentID = volume->metadataDir;
          ASCIIToUnicode(pathBuffer, &nkey.nodeName);
          nkey.keyLength = sizeof(nkey.parentID) +
                           sizeof(nkey.nodeName.length) +
                           (sizeof(uint16_t) * nkey.nodeName.length);

          toReturn = (HFSPlusCatalogRecord *)search(
              volume->catalogTree, (BTKey *)(&nkey), &exact, NULL, NULL);
...
HFSPlusCatalogRecord *getLinkTarget(HFSPlusCatalogRecord *record,
  ...
  if (record->recordType == kHFSPlusFileRecord &&
      (((HFSPlusCatalogFile *)record)->permissions.fileMode & S_IFLNK) ==
          S_IFLNK) {
    io = openRawFile(((HFSPlusCatalogFile *)record)->fileID,
                     &(((HFSPlusCatalogFile *)record)->dataFork), record,
                     volume);
    READ(io, 0, (((HFSPlusCatalogFile *)record)->dataFork).logicalSize,
         pathBuffer);
    CLOSE(io);
    pathBuffer[(((HFSPlusCatalogFile *)record)->dataFork).logicalSize] = '\0';
    toReturn =
        getRecordFromPath3(pathBuffer, volume, NULL, key, TRUE, TRUE, parentID);
    free(record);
    return toReturn;
  } else if (record->recordType == kHFSPlusFileRecord &&
             (((HFSPlusCatalogFile *)record)->userInfo.fileType) ==
                 kHardLinkFileType) {
    sprintf(pathBuffer, "iNode%d",
            ((HFSPlusCatalogFile *)record)->permissions.special.iNodeNum);
    nkey.parentID = volume->metadataDir;
    ASCIIToUnicode(pathBuffer, &nkey.nodeName);
    nkey.keyLength = sizeof(nkey.parentID) + sizeof(nkey.nodeName.length) +
                     (sizeof(uint16_t) * nkey.nodeName.length);
...
static const uint16_t METADATA_DIR[] = {0,   0,   0,   0,   'H', 'F', 'S',
                                        '+', ' ', 'P', 'r', 'i', 'v', 'a',
                                        't', 'e', ' ', 'D', 'a', 't', 'a'};
...
HFSCatalogNodeID getMetadataDirectoryID(Volume *volume) {
  ...
  key.keyLength =
      sizeof(key.parentID) + sizeof(key.nodeName.length) + sizeof(METADATA_DIR);
  key.parentID = kHFSRootFolderID;
  memcpy(key.nodeName.unicode, METADATA_DIR, sizeof(METADATA_DIR));

  record = (HFSPlusCatalogFolder *)search(volume->catalogTree, (BTKey *)(&key),
                                          &exact, NULL, NULL);
  id = record->folderID;
...
HFSPlusCatalogRecord *getRecordFromPath2(const char *path, Volume *volume,
                                         ...
                                         char traverse) {
  return getRecordFromPath3(path, volume, name, retKey, TRUE, TRUE,
                            kHFSRootFolderID);
}
...
HFSPlusCatalogRecord *getRecordFromPath3(const char *path, Volume *volume,
  ...
  if (path[0] == '\0' || (path[0] == '/' && path[1] == '\0')) {
    ...
    record = (HFSPlusCatalogRecord *)search(
        volume->catalogTree, (BTKey *)(&key), &exact, NULL, NULL);
  ...
  for (word = (char *)strtok(myPath, "/"); word && (word < pathLimit);
       word = ((word + strlen(word) + 1) < pathLimit)
                  ? (char *)strtok(word + strlen(word) + 1, "/")
                  : NULL) {
    ...
    key.keyLength = sizeof(key.parentID) + sizeof(key.nodeName.length) +
                    (sizeof(uint16_t) * key.nodeName.length);
    record = (HFSPlusCatalogRecord *)search(
        volume->catalogTree, (BTKey *)(&key), &exact, NULL, NULL);
...
int updateCatalog(Volume *volume, HFSPlusCatalogRecord *catalogRecord) {
  ...
  case kHFSPlusFolderRecord:
    ...
    free(record);
    return addToBTree(volume->catalogTree, (BTKey *)(&key),
                      sizeof(HFSPlusCatalogFolder), (unsigned char *)(&folder));
    break;
  ...
  case kHFSPlusFileRecord:
    ...
    free(record);
    return addToBTree(volume->catalogTree, (BTKey *)(&key),
                      sizeof(HFSPlusCatalogFile), (unsigned char *)(&file));
    break;
  }
...
int move(const char *source, const char *dest, Volume *volume) {
  ...
  srcRec = getRecordFromPath3(source, volume, NULL, &srcKey, TRUE, FALSE,
                              kHFSRootFolderID);
  if (srcRec == NULL) {
    free(srcRec);
    return FALSE;
  ...
  srcFolderRec =
      (HFSPlusCatalogFolder *)getRecordByCNID(srcKey.parentID, volume);

  if (srcFolderRec == NULL ||
      srcFolderRec->recordType != kHFSPlusFolderRecord) {
    free(srcRec);
    free(srcFolderRec);
    return FALSE;
  }
  ...
  if (lastSeparator == NULL) {
    destRec =
        (HFSPlusCatalogFolder *)getRecordFromPath("/", volume, NULL, NULL);
    destName = destPath;
  } else {
    destName = lastSeparator + 1;
    *lastSeparator = '\0';
    destRec =
        (HFSPlusCatalogFolder *)getRecordFromPath(destPath, volume, NULL, NULL);
  ...
  threadLength = sizeof(uint16_t) + sizeof(uint16_t) + sizeof(uint32_t) +
                 sizeof(uint16_t) +
                 (sizeof(uint16_t) * destKey.nodeName.length);
  ...
  destKey.keyLength = sizeof(uint32_t) + sizeof(uint16_t) +
                      (sizeof(uint16_t) * destKey.nodeName.length);
  ...
  case kHFSPlusFolderRecord:
    ...
    addToBTree(volume->catalogTree, (BTKey *)(&destKey),
  ...
  destKey.keyLength =
      sizeof(destKey.parentID) + sizeof(destKey.nodeName.length);

  flipCatalogThread(thread, TRUE);
  addToBTree(volume->catalogTree, (BTKey *)(&destKey), threadLength,
...
int removeFile(const char *fileName, Volume *volume) {
  ...
  record = getRecordFromPath3(fileName, volume, NULL, &key, TRUE, FALSE,
                              kHFSRootFolderID);
  if (record != NULL) {
    parentFolder =
        (HFSPlusCatalogFolder *)getRecordByCNID(key.parentID, volume);
    ...
    if (record->recordType == kHFSPlusFileRecord) {
      ...
      removeFromBTree(volume->catalogTree, (BTKey *)(&key));
      XAttrList *next;
      XAttrList *attrs = getAllExtendedAttributes(
          ((HFSPlusCatalogFile *)record)->fileID, volume);
      if (attrs != NULL) {
        while (attrs != NULL) {
          next = attrs->next;
          unsetAttribute(volume, ((HFSPlusCatalogFile *)record)->fileID,
                         attrs->name);
          free(attrs->name);
          free(attrs);
          attrs = next;
      ...
      if (((HFSPlusCatalogFolder *)record)->valence > 0) {
        ...
        XAttrList *next;
        XAttrList *attrs = getAllExtendedAttributes(
            ((HFSPlusCatalogFolder *)record)->folderID, volume);
        if (attrs != NULL) {
          while (attrs != NULL) {
            next = attrs->next;
            unsetAttribute(volume, ((HFSPlusCatalogFolder *)record)->folderID,
                           attrs->name);
            free(attrs->name);
            free(attrs);
            attrs = next;
...
int makeSymlink(const char *pathName, const char *target, Volume *volume) {
  ...
  record = (HFSPlusCatalogFile *)getRecordFromPath3(
      pathName, volume, NULL, NULL, TRUE, FALSE, kHFSRootFolderID);

  if (!record) {
    newFile(pathName, volume);
    record =
        (HFSPlusCatalogFile *)getRecordFromPath(pathName, volume, NULL, NULL);
    if (!record) {
      return FALSE;
    }
  ...
  io = openRawFile(record->fileID, &record->dataFork,
                   (HFSPlusCatalogRecord *)record, volume);
...
HFSCatalogNodeID newFolder(const char *pathName, Volume *volume) {
  ...
  if (lastSeparator == NULL) {
    ...
    *lastSeparator = '\0';
    parentFolder =
        (HFSPlusCatalogFolder *)getRecordFromPath(path, volume, NULL, NULL);
  }

  if (parentFolder == NULL ||
      parentFolder->recordType != kHFSPlusFolderRecord) {
    free(path);
    free(parentFolder);
    return FALSE;
  }
  ...
  threadLength = sizeof(thread.recordType) + sizeof(thread.reserved) +
                 sizeof(thread.parentID) + STR_SIZE(thread.nodeName);
  flipCatalogThread(&thread, TRUE);
  flipCatalogFolder(&folder);

  ASSERT(addToBTree(volume->catalogTree, (BTKey *)(&key),
                    sizeof(HFSPlusCatalogFolder), (unsigned char *)(&folder)),
         "addToBTree");
  key.nodeName.length = 0;
  key.parentID = newFolderID;
  key.keyLength = sizeof(key.parentID) + sizeof(key.nodeName.length);
  ASSERT(addToBTree(volume->catalogTree, (BTKey *)(&key), threadLength,
                    (unsigned char *)(&thread)),
         "addToBTree");
...
int chmodFile(const char *pathName, int mode, Volume *volume) {
  ...
  if (record->recordType == kHFSPlusFolderRecord) {
    ((HFSPlusCatalogFolder *)record)->permissions.fileMode =
        (((HFSPlusCatalogFolder *)record)->permissions.fileMode & 0770000) |
        mode;
  } else if (record->recordType == kHFSPlusFileRecord) {
    ((HFSPlusCatalogFile *)record)->permissions.fileMode =
        (((HFSPlusCatalogFolder *)record)->permissions.fileMode & 0770000) |
        mode;
  } else {
    return FALSE;
...
BTree *openCatalogTree(io_func *file) {
  ...
  btree = openBTree(file, &catalogCompare, &catalogKeyRead, &catalogKeyWrite,
                    &catalogKeyPrint, &catalogDataRead);
