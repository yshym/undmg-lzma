...
static BTKey *attrKeyRead(off_t offset, io_func *io) {
  ...
  if (!READ(io, offset + UNICODE_START, key->name.length * sizeof(uint16_t),
...
static BTKey *attrDataRead(off_t offset, io_func *io) {
  ...
  case kHFSPlusAttrInlineData:
    ...
    record = realloc(record, sizeof(HFSPlusAttrData) +
                                 ((HFSPlusAttrData *)record)->size);
    if (!READ(io, offset + sizeof(HFSPlusAttrData),
              ((HFSPlusAttrData *)record)->size,
              ((HFSPlusAttrData *)record)->data)) {
      return NULL;
    }
...
static int updateAttributes(Volume *volume, HFSPlusAttrKey *skey,
                            HFSPlusAttrRecord *srecord) {
  HFSPlusAttrKey key;
  HFSPlusAttrRecord *record;
  int ret, len;
  ...
  case kHFSPlusAttrInlineData:
    ...
    ret = addToBTree(volume->attrTree, (BTKey *)(&key), len,
  ...
  case kHFSPlusAttrForkData:
    ...
    flipAttrForkData((HFSPlusAttrForkData *)record);
    removeFromBTree(volume->attrTree, (BTKey *)(&key));
    ret = addToBTree(volume->attrTree, (BTKey *)(&key),
                     sizeof(HFSPlusAttrForkData), (unsigned char *)record);
    free(record);
    break;
  case kHFSPlusAttrExtents:
    ...
    removeFromBTree(volume->attrTree, (BTKey *)(&key));
    ret = addToBTree(volume->attrTree, (BTKey *)(&key),
                     sizeof(HFSPlusAttrExtents), (unsigned char *)record);
    free(record);
    break;
  }
...
size_t getAttribute(Volume *volume, uint32_t fileID, const char *name,
  ...
  key.keyLength = sizeof(HFSPlusAttrKey) - sizeof(HFSUniStr255) +
                  sizeof(key.name.length) +
                  (sizeof(uint16_t) * key.name.length);

  *data = NULL;

  record = (HFSPlusAttrRecord *)search(volume->attrTree, (BTKey *)(&key),
                                       &exact, NULL, NULL);
...
XAttrList *getAllExtendedAttributes(HFSCatalogNodeID CNID, Volume *volume) {
  ...
  record = (HFSPlusAttrRecord *)search(tree, (BTKey *)(&key), NULL, &nodeNumber,
                                       &recordNumber);
  if (record == NULL) {
    return NULL;
  }
...
BTree *openAttributesTree(io_func *file) {
  return openBTree(file, &attrCompare, &attrKeyRead, &attrKeyWrite,
                   &attrKeyPrint, &attrDataRead);
}
