...
static void cacheRun(DMG* dmg, BLKXTable* blkx, int run) {
  ...
  switch(blkx->runs[run].type) {
		case BLOCK_ZLIB:
			...
			ASSERT((zstrm.avail_in = dmg->dmg->read(dmg->dmg, inBuffer, blkx->runs[run].compLength)) == blkx->runs[run].compLength, "fread");
			zstrm.next_in = (unsigned char*) inBuffer;

			do {
				zstrm.avail_out = bufferSize;
				zstrm.next_out = (unsigned char*) dmg->runData;
				ASSERT((ret = inflate(&zstrm, Z_NO_FLUSH)) != Z_STREAM_ERROR, "inflate/Z_STREAM_ERROR");
		...
		case BLOCK_BZ2:
			...
			do {
				...
				ASSERT((ret = BZ2_bzDecompress(&bzstrm)) != BZ_DATA_ERROR, "bzDecompress/Z_STREAM_ERROR");
