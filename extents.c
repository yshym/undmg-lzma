...
static inline void
flipExtentDescriptor(HFSPlusExtentDescriptor *extentDescriptor) {
  ...
  if (left->forkType < right->forkType) {
    ...
    if (left->fileID < right->fileID) {
      ...
      if (left->startBlock < right->startBlock) {
        ...
        /* do a safety check on key length. Otherwise, bad things may happen
         * later on when we try to add or remove with this key */
...
BTree *openExtentsTree(io_func *file) {
  return openBTree(file, &extentCompare, &extentKeyRead, &extentKeyWrite,
                   &extentKeyPrint, &extentDataRead);
}
